var DEFAULT_ELEMENT_NAME = '(anonymous)';
...
function Element(definition, options) {
  options = options || {};
  this.name = options.name || DEFAULT_ELEMENT_NAME;
...
Element.copyDefaults = function(target, source) {
  var props = ['parent', 'selector', 'locateStrategy', 'index'];
  ...
  // special case for names since they have a default
  if (!target.name || target.name === DEFAULT_ELEMENT_NAME) {
    target.name = source.name || DEFAULT_ELEMENT_NAME;
  }
};
