  ...
  function makeWrappedCommand(parent, commandFn, commandName, isChaiAssertion) {
    return function() {
      ...
      if (inputElement.hasElementSelector()) {
        ...
        // carry over the original selector index, if it has one,
        // so that it can be applied on top of the target element.
        // this would override any index in the original definition
        // but also allows it to be configurable, to an extent

        if (inputElement.index != undefined) {
          elementOrSection.index = inputElement.index;
        }
