...
module.exports = new (function() {
  ...
  function makeWrappedCommand(parent, commandFn, commandName, isChaiAssertion) {
    return function() {
      ...
      var isElementCommand = Element.isElementSelector(args[0]);

      if (isElementCommand) {

        var element = Element.fromSelector(args[0]);
        var getter = (isChaiAssertion && commandName === 'section') ? getSection : getElement;
        var elementOrSection = getter(parent, element.selector);
        var ancestors = getAncestorsWithElement(elementOrSection);

        if (ancestors.length > 1) {
          elementOrSection.selector = ancestors;
          elementOrSection.locateStrategy = 'recursion';
        }

        // TODO: copy any non-selector properties in element to elementOrSection.
        // allow null values in Elements instances; should parse parse into a generic object first?
        // TODO: circular JSON error - element objects are getting into un-wrapped
        // protocol elements() command without wrapper (waitFor commands, and potentially 
        // anyone else calling directly into elements)

        args[0] = elementOrSection;
      }
