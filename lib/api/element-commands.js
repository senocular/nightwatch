...
module.exports = function(client) {
  ...
  function CommandAction(using, value, protocolAction, args, callback, originalStackTrace) {
    ...
    // as long as we're not using recursion, we attempt to parse
    // complex data out of the selector value.

    if (using !== 'recursion') {

      // array value format:
      // [value, elementIndex (opt), using (opt)]

      if (Array.isArray(value)) {

        if (value.length > 1) {
          elementIndex = parseInt(value[1], 10);
        }
        if (value.length > 2) {
          using = value[2];
        }
      ...
      // default value format as selector
