...
module.exports = function(client) {
  ...
  /**
   * Parses the value/selector parameter of an element command. The standard
   ...
   */
  function parseSearchTarget(using, value) {
    ...
    if (Array.isArray(value)) {
      ...
      searchProps.value = String(value[0]);

      if (value.length > 1) {
        searchProps.index = parseInt(value[1], 10);
      }

      if (value.length > 2) {
        searchProps.using = String(value[2]);
      }
    ...
    // { selector: 'div', index (opt): 0, locateStrategy (opt): 'css selector' }
      ...
      searchProps.value = String(value.selector);

      if ('locateStrategy' in value) {
        searchProps.using = String(value.locateStrategy);
      }

      if ('index' in value) {
        searchProps.index = parseInt(value.index, 10);
      }
    ...
    // '#my-id div.my-class'
      ...
      searchProps.value = String(value);
    }
  ...
  function CommandAction(using, value, protocolAction, args, callback, originalStackTrace) {
    ...
    // parse complex data out of the selector value.

    var searchProps = parseSearchTarget(using, value);
    using = searchProps.using;
    value = searchProps.value;
    var elementIndex = searchProps.index;

    // selectively decide between element or elements based on if we neeed
    // to grab an element by index. Waste not, though I'm not sure how much
    // an optimization it really is to use element() over elements() if we're
    // just after the first element.

    var elementCommand = elementIndex ? 'elements' : 'element';

    var $this = this;
    var el = Protocol[elementCommand](using, value, function(result) {

      // if an element index is specified, we need to make sure its
      // within the range of elements found. If not, we patch the
      // status of the result to be non-0, marking the request a failure

      if (elementIndex && result.status === 0) {
        if (result.value.length <= elementIndex) {
          result.status = -1;
        }
      }
      ...
      if (result.status !== 0) {
        ...
        // the elements() request when elementIndex is non-0 returns an array
        // of ELEMENT values when element() (elementIndex = 0) is just one ELEMENT

        result = elementIndex ? result.value[elementIndex].ELEMENT : result.value.ELEMENT;
