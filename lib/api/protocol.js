...
module.exports = function(Nightwatch) {
  ...
  function elements(elem, callback) {
    ...
    if (!check.test(elem.locateStrategy)) {
      throw new Error('Please provide any of the following using strings as the first parameter: ' +
        'class name, css selector, id, name, link text, partial link text, tag name, or xpath. Given: ' + elem.locateStrategy);
    }
  ...
  function createIndexedElementCallback(element, callback) {
    return function (result) {

      var usingIndex = element.index != undefined;
      if (usingIndex && result && result.status === 0) {

        // if an element index is specified, we need to make sure its
        // within the range of elements found. If not, we patch the status
        // of the result to be non-0, marking the request a failure

        if (result.value.length <= element.index) {
          result.status = -1;
        } else {
          ...
          result.value = [result.value[element.index]];
        }
      }
  ...
  Actions.elementIdElements = function(id, using, value, callback) {
    ...
    var strategies = ['class name', 'css selector', 'id', 'name', 'link text',
      'partial link text', 'tag name', 'xpath'];
    using = String(elem.locateStrategy).toLocaleLowerCase();
