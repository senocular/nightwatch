...
ElementsByRecursion.prototype.command = function(elements, callback) {
  ...
  var el = this.protocol.elements(topElement.locateStrategy, topElement, function checkResult() {
    ...
    if (nextElement) {
      ...
      result.value.forEach(function(el) {
        var p = deferredElementIdElements(el.ELEMENT, nextElement.locateStrategy, nextElement, checkResult);
        promises.push(p);
      });
