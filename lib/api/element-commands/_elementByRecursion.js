...
ElementByRecursion.prototype.command = function(elements, callback) {
  ...
  var el = this.protocol.element(topElement.locateStrategy, topElement, function checkResult(result) {
    if (result.status !== 0) {
      callback(result);
      ...
      if (nextElement) {
        self.protocol.elementIdElement(parentId, nextElement.locateStrategy, nextElement, checkResult);
      } else {
        callback(result);
        self.emit('complete', el, self);
      }
