...
ClientManager.prototype.start = function(done) {
  console.log('CLIENT START', new Error('GET_STACK').stack)
  var self = this;
  ...
  this['@client'].once('nightwatch:finished', function(results, errors) {
    ...
    if (done) {
      if (results.failed > 0 || results.errors > 0) {
        ...
        results.lastError = null; // TODO: why? ClientManager is client's only client? This is set in assert()
      } else {
        done();
      }
    }
...
ClientManager.prototype.terminate = function(deferred) {
  this['@client'].terminate(deferred);
  return this;
};
...
ClientManager.prototype.emptyQueue = function() {
