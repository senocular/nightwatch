...
TestSuite.prototype.runTestMethod = function(methodName, context, methodFn, timeoutTime) {
  ...
  return this.makePromise(function testMethodPromise(done) {

    done = this.addTimeoutToDoneCallback(done, methodDesc, timeoutTime);

    var methodDoneCalled = false;
    var completeCalled = false;

    function completeMethod(err, errIsFailure) {
      self.addErrorToResults(err, errIsFailure);
    ...
    function methodDone(err) {
      methodDoneCalled = true;
      if (err || !self.client.shouldRestartQueue()) { // TODO: if sync done(err) then throw, that's a doublt - complete
        completeMethod(err, true);
      }
    }

    // TODO are these errors already logged? so don't pass error into complete?
    // Or do we need it for clearing queue?
    function queueDone() {
      if (methodDoneCalled) {
        completeMethod();
      }
    ...
    try {
      // catches sync and async throws
      this.currentCatchHandler = completeMethod;
      this.callTestMethod(methodFn, context, methodDone, singleParamUsesDone);
    } catch (err) {
      this.catchHandler(err);
      return;
    }
    ...
    if (this.client.terminated()) {
      completeMethod(); // failed sync assert; TODO: does this still spin up an END()?
    } else if (!completeCalled && this.client.shouldRestartQueue()) {
      this.startQueue(queueDone);
    }
  });
};
...
TestSuite.prototype.addErrorToResults = function(errs, errIsFailure) {
  if (errs) {
