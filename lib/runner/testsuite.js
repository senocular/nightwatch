...
TestSuite.prototype.setupClient = function() {
  ...
  this.client.on('error', function(err) {
    // TODO: Is this fatal? Instead catchHandler()? How does this affect in-process runs
    self.complete(err);
  });
...
TestSuite.prototype.runTestMethod = function(methodName, context, methodFn, timeoutTime) {
  ...
  return this.makePromise(function(done) {
    ...
    function steplog () { // TODO: cleanup
      var args = [].slice.call(arguments);
      args.unshift('STEP ' + methodDesc);
      //console.log.apply(console, args);
    }

    done = this.addTimeoutToDoneCallback(done, methodDesc, timeoutTime, function onTimeout(err) { steplog('TIMEOUT');
      completeMethod(err);
    });
    ...
    function completeMethod(err, isRejectErr) {         steplog('COMPLETE', 'hasError:', !!err);
      self.addErrorToResults(err); // for every complete attempt, even after first

      if (!completeCalled) {
        completeCalled = true;

        self.clearQueue(err, function onQueueCleared() { steplog('DONE');
          if (isRejectErr) {
            done(err);
          } else {
            done();
    ...
    function methodDone(err) {                          steplog('methodDone');
      methodDoneCalled = true;
      if (!self.client.shouldRestartQueue()) { // TODO: (?) will timeout if queue created async before done()
        completeMethod(err, true); // TODO: if an error, should we skip queue check and terminate? (check abortOnFailure)
      }                                                 else {steplog('methodDone WAITING...');}
    }

    function queueDone(err) {                           steplog('queueDone');
      if (methodDoneCalled) {
        completeMethod(err);
      }                                                 else {steplog('queueDone WAITING...');}
    }

    // catches async throws
    this.currentCatchHandler = completeMethod;

    try {                                               steplog('CALL');
      this.callTestMethod(methodFn, context, methodDone, singleParamUsesDone); steplog('CALLED');
    } catch (err) {                                     steplog('CALL CAUGHT', err);
      completeMethod(err);
      return;
    }

    if (this.client.terminated()) {                     steplog('CALL TERMINATED');
      // failed sync assert; TODO: does this still spin up an END()? Are errors logged? If so, does queueDone need err logging?
      completeMethod();
    } else if (!completeCalled && this.client.shouldRestartQueue()) { steplog('START QUEUE');
      this.client.start(queueDone); // TODO: done get called on('error')?? done(err) is "last error" - do we log that? (see client)
    }
                                                        steplog('PROMISE MADE');
  });
};
...
TestSuite.prototype.clearQueue = function(err, done) {
  ...
  if (err && this.options.start_session) { // TODO: can we have an already terminated queue here? Could we double-terminate?
    ...
    // FIXME: TOOD: start resets terminated breaking later is-terminated checks
    this.client
      .terminate(true) // TODO: sync, no-queue g.beforeEach throw calls terminate. Expected
      .start(done); // TODO: what happens when this on.errors while we wait? Do hooks need to check suite complete before running?
  } else {
    done();
  }
};
...
TestSuite.prototype.addTimeoutToDoneCallback = function(done, methodDesc, timeoutTime, errorHandler) {
  if (timeoutTime <= 0) {
    return done;
  }
  return Utils.setCallbackTimeout(done, methodDesc, timeoutTime, errorHandler, function(timeoutId) {
...
TestSuite.prototype.complete = function(err) {
  ...
  // TODO: find callers (e.g. onError). This should not be called in the middle of a hook/test chain
  if (err) {
...
TestSuite.prototype.catchHandler = function(err) {
  if (this.currentCatchHandler) {
    this.currentCatchHandler(err, true);
  } else {
    // TODO: is this disruptive to any existing chains? Where are there gaps when this is called?
    this.complete(err);
  }
};
