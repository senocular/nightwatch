...
function TestCase(suite, testFn, numRetries, maxRetries) {
  ...
  this._deferredNext = null; // test case call deferred
  this.running = false;
...
TestCase.prototype.run = function () {
  ...
  this.suite.beforeEach()
    .then(function() {
      ...
      self.suite.client.once('complete', function() {
      ...
      try {
        if (self.suite.client.options.start_session) {
          // TODO: does this mean if you manage your own session, you'll get a timeout error for test cases?
          //    same with resolve above?
...
TestCase.prototype.doneCallback = function(err) {
  ...
  if (this._deferredNext) {
    this._deferredNext.resolve({
