...
function processListener() {
  ...
  process.on('uncaughtException', function (err) {
    console.log('PROCESS UNCAUGHT', err);
    if (currentTestSuite) {
      currentTestSuite.catchHandler(err);
...
Runner.prototype.runTestModule = function(modulePath, fullPaths) {
  ...
  try {
    currentTestSuite = this.currentTestSuite = new TestSuite(modulePath, fullPaths, this.options, this.additionalOpts);

    moduleKey = this.currentTestSuite.getReportKey();
  ...
  return this.currentTestSuite
    ...
    .catch(function (err) {
      console.log('UNCAUGHT RUN ERROR', err)
    });
};
  ...
  Runner.readPaths(this.testSource, this.options)
    .spread(function(modulePaths, fullPaths) {
      ...
      function runNextModule() {
        ...
        return self.runTestModule(modulePath, fullPaths)
          .then(function(testResults) {
            if (modulePaths.length) {
              return runNextModule();
            }
            return self.reportTestResults(testResults);
          });
      }
      ...
      self.doneCb(err); // TODO: did pass "false" for results. Its this checked for?
      deferred.resolve();
    })
    .catch(function(err) { // error in doneCb
      deferred.reject(err);
    })
...
Runner.prototype.reportTestResults = function (testResults) {
  var deferred = Q.defer();
  var self = this;

  var reporter = new Reporter(this.globalResults, testResults, this.globalStartTime, {
    output_folder : this.additionalOpts.output_folder,
    filename_prefix : this.options.report_prefix,
    globals : this.options.globals,
    reporter : this.additionalOpts.reporter,
    start_session : this.options.start_session
  });

  if (this.options.output) {
    reporter.printTotalResults(this.globalResults, testResults);
  }
