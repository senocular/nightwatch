...
module.exports = new (function() {
   ...
   * @param {string} [dirPath]
   * @param {object} [parent]
   * @param {array} [recursiveCommands]
   */
  function loadCustomCommands(dirPath, parent, recursiveCommands) {
    if (!custom_commands_path && !dirPath) {
      return;
    }

    // loaded commands of this call and any recursive
    // calls are added to the same commands list

    var commands = recursiveCommands || [];
    ...
    if (Array.isArray(dirPath)) {
      ...
      dirPath.forEach(function(folder) {
        loadCustomCommands(folder, parent, commands);
      });
      ...
      commandFiles.forEach(function(file) {
        ...
        if (fs.lstatSync(fullPath).isDirectory()) {
          ...
          loadCustomCommands(pathFolder, parent[file], commands);
          ...
          commands.push({
            ...
            path: fullPath,
            parent: parent,
            module: loadCommandModule(commandModule, client.api)
          });
        }
      });
    ...
    // if this was the original call (non-recursive)
    // add the collected commands

    if (!recursiveCommands) {

      // remove any duplicates (by name), preferring the
      // commands added last in the command list

      var commandIsAdded = {};
      commands = commands.reverse().filter(function(command) {
        if (!commandIsAdded[command.name]) {
          commandIsAdded[command.name] = command;
          return true;
        }
        return false;
      });
      ...
      commands.forEach(function(command){
        addCommand(command.name, command.module.command, command.module.context, command.parent, true);
      });
